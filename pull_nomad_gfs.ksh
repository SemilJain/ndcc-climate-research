...
while [[ $CUR_FHR -le $MAX_FHR ]]; do
  ...
  CUR_GRB_FILE=gfs.t${INIT_H}z.pgrb2.0p25.f$(printf "%03d" $CUR_FHR)
  CUR_GRB_RENAME=${INIT_YMD}_i${INIT_H}_f${CUR_FHR}_gfs0p25.grb2

  # Pull the files
  echo "RUNNING: time curl https://nomads.ncep.noaa.gov/pub/data/nccf/com/gfs/prod/gfs.${INIT_YMD}/${INIT_H}/atmos/${CUR_GRB_FILE}"
  # time wget https://noaa-gfs-bdp-pds.s3.amazonaws.com/gfs.${INIT_YMD}/${INIT_H}/atmos/${CUR_GRB_FILE}
  time curl -LO https://nomads.ncep.noaa.gov/pub/data/nccf/com/gfs/prod/gfs.${INIT_YMD}/${INIT_H}/atmos/${CUR_GRB_FILE}
